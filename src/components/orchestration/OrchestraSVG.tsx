...
const getHighlightedGroups = (
    ...
    highlightList.forEach((item) => {
        if (item in categoryMap) {
            categoryMap[
                item as InstrumentCategory
            ].forEach((group) => groups.add(group));
        } else {
            groups.add(item as InstrumentGroup);
        }
    });
...
> = {
    ...
    brass: {
        d: "M909.098 421.543C931.411 408.87 939.238 380.192 923.717 359.758C891.312 317.098 850.328 281.227 803.138 254.479C755.603 227.537 703.084 210.595 648.898 204.524C623.65 201.695 603.026 222.794 603.072 248.2V248.2C603.117 273.604 623.858 294.008 648.99 297.714C686.635 303.265 723.033 315.708 756.249 334.535C789.507 353.385 818.735 378.16 842.513 407.463C858.36 426.993 885.896 434.72 907.766 422.299L909.098 421.543Z",
        fill: "#D9C041",
    },

    horns: {
        d: "M298.286 421.543C275.973 408.87 268.146 380.192 283.667 359.758C315.915 317.303 356.664 281.57 403.573 254.862C450.828 227.957 503.041 210.947 556.946 204.7C582.181 201.775 602.884 222.594 602.939 247.998L602.944 250.23C602.999 275.637 582.334 295.925 557.217 299.751C520.121 305.402 484.276 317.791 451.557 336.419C418.794 355.073 389.985 379.518 366.507 408.396C350.642 427.91 323.107 435.64 301.238 423.219L298.286 421.543Z",
        fill: "#D9A441",
    },
...
const OrchestraSVG = ({
    ...
    const highlightedGroups =
        getHighlightedGroups(highlightList);

    const hasHighlights =
        highlightedGroups.size > 0;

    const getGroupClassName = (
        groupId: InstrumentGroup,
    ): string => {
